...
const convertToJSON = (config, srcPath, docObjects) => {
  const [_, ...tailPath] = srcPath.replace(".js", "").split("/");
  const [filename, ..._rest] = tailPath.slice().reverse();
  const rawPath = [config.dir, ...tailPath].slice(0, -1);
  const path = rawPath.join("/");
  return {
    string: JSON.stringify(docObjects),
    path,
...
const doc = ({ references, state, babel, config }) => {
  ...
  const srcPath = getFilePath(state, config);
  ...
  const jsonFile = convertToJSON(config, srcPath, docObjects);
  writeFile(jsonFile);
